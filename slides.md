...
左側（設計）の各工程が、右側（検証）の確認対象と **1対1で対応する**
...
| トレーサビリティ | 縦：上位設計が下位設計に正しく反映されているか | 横：要件が最終的に満たされているか |
| 確認方法 | レビュー・トレーサビリティマトリクス | テスト（実際に動かす） |
| 確認対象 | 設計・仕様通りに実装されているか | 要件がそもそも目的に合っているか |
| 例 | 「詳細設計通りにコードが書かれているか」 | 「その警告はドライバーの安全に本当に役立つか」 |

- Verification だけ通っても、*作るものが間違っていれば意味がない*
- だから Validation でお客さま・現場の視点から確認する

> 縦（各工程間の整合）と横（設計↔検証の対応）、両方のトレーサビリティで品質を担保する
